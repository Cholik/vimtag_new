...
#define kArrowHorizontalPadding 8.f
...
#define kDividerColor [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:0.15f]
#define kDividerColor_luxcam [UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:0.55f]

// BACKGROUND GRADIENT

//bottom color white in gradient bg
#define kGradientBottomColor [UIColor colorWithWhite:0.2f alpha:kBoxAlpha]
#define kGradientBottomColor_luxcam [UIColor colorWithWhite:1.0f alpha:kBoxAlpha_luxcam]

//top color white value in gradient bg
#define kGradientTopColor [UIColor colorWithWhite:0.2f alpha:kBoxAlpha]
#define kGradientTopColor_luxcam [UIColor colorWithWhite:1.0f alpha:kBoxAlpha_luxcam]

// TITLE GRADIENT

//bottom color white value in title gradient bg
#define kGradientTitleBottomColor [UIColor colorWithWhite:0.2f alpha:kBoxAlpha]
#define kGradientTitleBottomColor_luxcam [UIColor colorWithWhite:1.0f alpha:kBoxAlpha_luxcam]


//top color white value in title gradient bg
#define kGradientTitleTopColor [UIColor colorWithWhite:0.2f alpha:kBoxAlpha]
#define kGradientTitleTopColor_luxcam [UIColor colorWithWhite:1.0f alpha:kBoxAlpha_luxcam]

// FONTS

//normal text font
//#define kTextFont [UIFont fontWithName:@"HelveticaNeue" size:20.f]
#define kTextFont [UIFont fontWithName:@"HelveticaNeue" size:14.0f]

//normal text color
#define kTextColor [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:1]
#define kTextColor_luxcam [UIColor colorWithRed:0 green:0 blue:0 alpha:1]
...
#define kTitleFont [UIFont fontWithName:@"Arial" size:17.0f]

//title text color
#define kTitleColor [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:1]
#define kTitleColor_luxcam [UIColor colorWithRed:0 green:0 blue:0 alpha:1]
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withViewArray:(NSArray *)viewArray {
    ...
    titleLabel.textColor = (self.configuration.is_whiteBackground?kTitleColor_luxcam : kTitleColor);
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withStringArray:(NSArray *)stringArray {
    ...
    for(NSString *string in stringArray) {
        ...
        label.textColor = (self.configuration.is_whiteBackground?kTextColor_luxcam : kTextColor);
        label.text = string;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withStringArray:(NSArray *)stringArray {
    ...
    for(NSString *string in stringArray) {
        ...
        {
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 1.0)];
...
//            CGSize titleSize = [title sizeWithFont:kTitleFont];
            CGSize textSize = [string sizeWithFont:font];
            UILabel *label ;
            if([NSLocalizedString(@"mcs_exit",nil) isEqualToString:string]
               || [NSLocalizedString(@"mcs_password",nil) isEqualToString:string]
               || [NSLocalizedString(@"mcs_add_device",nil) isEqualToString:string]
               || [NSLocalizedString(@"mcs_settings",nil) isEqualToString:string]
               || [NSLocalizedString(@"mcs_warranty_registration",nil) isEqualToString:string]
               || [NSLocalizedString(@"mcs_get_more_cameras",nil) isEqualToString:string]
               || [NSLocalizedString(@"mcs_help_feedback",nil) isEqualToString:string]
               || [NSLocalizedString(@"mcs_my_folder",nil) isEqualToString:string])
            {
                label = [[UILabel alloc] initWithFrame:CGRectMake(0.f, 0.f, textSize.width, textSize.height)];
                label.backgroundColor = [UIColor clearColor];
                label.font = kTitleFont;
                //label.textAlignment = UITextAlignmentCenter;
                label.textColor = (self.configuration.is_whiteBackground?kTitleColor_luxcam : kTitleColor);
                label.text = [NSString stringWithFormat:@"  %@",string];
                float totalWidth = textSize.width;
                self.titleView = label;
//                label.frame = CGRectMake(totalWidth*0.5f - titleSize.width*0.5f, 0, titleSize.width, titleSize.height);
                label.frame = CGRectMake(totalWidth*0.5f - textSize.width*0.5f, 0, textSize.width+50, textSize.height+10);
            }
            else
            {
//                label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, textSize.width, textSize.height)];
                label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, textSize.width+10, textSize.height+10)];
                label.backgroundColor = [UIColor clearColor];
                label.font = font;
                label.textAlignment = NSTextAlignmentLeft;
                label.textColor = (self.configuration.is_whiteBackground?kTextColor_luxcam : kTextColor);
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withContentView:(UIView *)cView {
    ...
    if (self.app.is_sereneViewer && ([UIScreen mainScreen].applicationFrame.size.height > [UIScreen mainScreen].applicationFrame.size.width || UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)) {
        _helpButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [_helpButton setBackgroundImage:[UIImage imageNamed:@"help.png"] forState:UIControlStateNormal];
        _helpButton.frame = CGRectMake(topView.frame.size.width - 60, topView.frame.size.height - 55, 60, 60);
        [topView addSubview:_helpButton];
    }
...
- (void)showImage:(UIImage *)image withMessage:(NSString *)msg {
    ...
    if(msg) {
        if([titleView isKindOfClass:[UILabel class]]) {
            ((UILabel *)titleView).text = [msg stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
            ((UILabel *)titleView).font = kTextFont;
        }
    }
...
//- (void)showError {
//    UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"popview_error"]];
//    imageView.alpha = 0.f;
...
//
//- (void)showSuccess {
//    UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"popview_success"]];
//    imageView.alpha = 0.f;
//    imageView.frame = CGRectMake(CGRectGetMidX(contentView.bounds) - 20.f, CGRectGetMidY(contentView.bounds) - 20.f + 20.f, 40.f, 40.f);
//    imageView.transform = CGAffineTransformMakeScale(0.1f, 0.1f);
//    
        ...
        if(CGRectContainsPoint(view.frame, point)) {
            ...
            if([view isKindOfClass:[UILabel class]]) {
                UILabel *label = (UILabel *)view;
                 label.backgroundColor = self.configuration.is_whiteBackground?[UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:1]:[UIColor colorWithRed:0.329 green:0.341 blue:0.353 alpha:1];
...
//    }
    ...
    NSArray* gradientColors = [NSArray arrayWithObjects:
                               (id)(self.configuration.is_whiteBackground?kGradientTopColor_luxcam.CGColor:kGradientTopColor.CGColor),
                               (id)(self.configuration.is_whiteBackground?kGradientBottomColor_luxcam.CGColor:kGradientBottomColor.CGColor), nil];
    ...
    //Draw the title background
    {
        ...
        if(titleBGHeight > 0.f) {
            ...
            //// Gradient Declarations
            NSArray* gradientColors = [NSArray arrayWithObjects:
                                       (id)(self.configuration.is_whiteBackground?kGradientTitleTopColor_luxcam.CGColor:kGradientTitleTopColor.CGColor),
                                       (id)(self.configuration.is_whiteBackground?kGradientTitleBottomColor_luxcam.CGColor:kGradientTitleBottomColor.CGColor), nil];
            CGFloat gradientLocations[] = {0, 1};
            CGGradientRef gradient = CGGradientCreateWithColors(colorSpace, (CFArrayRef)gradientColors, gradientLocations);
        ...
        if(kShowDividersBetweenViews && showDividerRects) {
            if(dividerRects && dividerRects.count > 0) {
                for(NSValue *value in dividerRects) {
                    ...
                    self.configuration.is_whiteBackground?[kDividerColor_luxcam setFill]:[kDividerColor setFill];
                    [dividerPath fill];
                }
            }
        }
