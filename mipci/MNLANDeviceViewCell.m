...
@implementation MNLANDeviceViewCell
...
-(void)localLoadWebImage:(BOOL)isLoadCache devType:(NSString *)devType deviceID:(NSString *)currentDeviceID
{
    ...
    if ([devType  isEqualToString: @"BOX"]) {
        UIImage *image;
        if (self.app.is_vimtag) {
            self.backgroundImageView.image = [UIImage imageNamed:@"vt_box_placeholder.png"];
            image = [UIImage imageNamed:@"vt_box_placeholder.png"];
        }
        else if (self.app.is_ebitcam)
        {
            self.backgroundImageView.image = [UIImage imageNamed:@"eb_cellBg.png"];
            image = [UIImage imageNamed:@"eb_cellBg.png"];
        }
        else if (self.app.is_mipc)
        {
            self.backgroundImageView.image = [UIImage imageNamed:@"mi_cellBg.png"];
            image = [UIImage imageNamed:@"mi_cellBg.png"];
        }
        else
        {
            self.backgroundImageView.image = [UIImage imageNamed:@"box_placeholder.png"];
            image = [UIImage imageNamed:@"box_placeholder.png"];
        }
        // NSString *imagePath = [self localImagePathByDeviceID:_deviceID];
    ...
    else if ([devType  isEqualToString: @"IPC"])
    {
        if (self.app.is_vimtag)
        {
    ...
    if (isLoadCache) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            NSString *imagePath = [weakSelf lanImagePathByDeviceID:currentDeviceID];
...
#pragma mark - Utils
- (NSString*)lanImagePathByDeviceID:(NSString *)deviceID
{
    NSString *path = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask, YES) firstObject];
    NSString *imageDirectory  = [path stringByAppendingPathComponent:@"photos/lanDeviceCell"];
