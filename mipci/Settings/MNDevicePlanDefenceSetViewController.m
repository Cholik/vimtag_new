//
//  MNDevicePlanDefenceSetViewController.m
//  mipci
//
...
@interface MNDevicePlanDefenceSetViewController ()
...
@property (assign, nonatomic) BOOL enableResult;
@property (strong, nonatomic) NSMutableArray *relatedArray;
@property (weak, nonatomic) MNConfiguration *configuration;
@property (strong, nonatomic) NSMutableArray *tmptimes;
@property (weak, nonatomic) AppDelegate *app;

@end

@implementation MNDevicePlanDefenceSetViewController
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        self.title = NSLocalizedString(@"mcs_scheduled_alerting", nil);
    }
...
- (void)initUI
{
    self.navigationItem.title = NSLocalizedString(@"mcs_scheduled_alerting", nil);
    ...
    _defenceStateTiteLabel.text = NSLocalizedString(@"mcs_enabled", nil);
    ...
    _defenceStateSwitch.onTintColor = self.configuration.switchTintColor;
}
...
- (void)viewDidLoad {
    ...
    _headTite = [NSArray arrayWithObjects:NSLocalizedString(@"mcs_scheduled_alerting_one", nil),
                 NSLocalizedString(@"mcs_scheduled_alerting_two", nil),
                 NSLocalizedString(@"mcs_scheduled_alerting_three", nil),
                 NSLocalizedString(@"mcs_scheduled_alerting_four", nil), nil];
    ...
    _isViewAppearing = YES;
    _tmptimes = [[NSMutableArray alloc] init];
    mcall_ctx_alert_task_get *ctx = [[mcall_ctx_alert_task_get alloc]init];
    ctx.target = self;
    ctx.on_event = @selector(alert_task_get_done:);
    ctx.sn = _deviceID;
    
    [_agent alert_task_get:ctx];
    [self loading:YES];
...
- (void)alert_task_get_done:(mcall_ret_alert_task_get *)ret
{
    [self loading:NO];
    ...
    if(nil == ret.result)
    {
        //alarm_action *alarm = ret
        _enableResult = ret.enableAlert;
        _defenceStateSwitch.on  = ret.enable;
        ...
        [_defenceStateSwitch sendActionsForControlEvents:UIControlEventValueChanged];
    }
    else
    {
...
- (void)alert_task_set_done:(mcall_ret_alert_task_set *)ret
{
    [self loading:NO];
    if (!_isViewAppearing)
    {
...
//        if (!wday_byte)continue;
    ...
    {
        if (self.tableView.numberOfSections == 6) {
            NSIndexSet *indexSet = [NSIndexSet indexSetWithIndexesInRange:range];
...
//    }
    ...
    mcall_ctx_alert_task_set *ctx = [[mcall_ctx_alert_task_set alloc] init];
    ctx.enableAlert = _enableResult;
    ctx.enable = _defenceStateSwitch.on;
    ctx.full_time = NO;
    ctx.times = times;
    ctx.target = self;
    ctx.sn = _deviceID;
    ctx.on_event = @selector(alert_task_set_done:);
    
    [self loading:YES];
    [_agent alert_task_set:ctx];
}
...
-(NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section{
    if (_defenceStateSwitch.on && section > 0 && section < 5)
    {
        return [_headTite objectAtIndex:section-1];
    }
    else
    {
        return nil;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section >= 1)
    {
        if (0 == indexPath.row)
        {
            _currentTimeLabel = self.startTimeLabels[indexPath.section - 1];
            [self createDatePickerWithMode:UIDatePickerModeCountDownTimer];
            
        }
        else if (1 == indexPath.row)
        {
            _currentTimeLabel = self.endTimeLabels[indexPath.section - 1];
            [self createDatePickerWithMode:UIDatePickerModeCountDownTimer];
        ...
        else if (2 == indexPath.row)
        {
            ...
            _currentDateLabel = self.dateTimeLabels[indexPath.section - 1];
            [self performSegueWithIdentifier:@"MNDefenceWeekListViewController" sender:[NSNumber numberWithInt:index]];
...
{
    if ([segue.identifier isEqualToString:@"MNDefenceWeekListViewController"]) {
        ...
        weekListViewController.devicePlanDefenceSetViewController = self;
    }
}
