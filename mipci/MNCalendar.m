...
#define kYello          [UIColor colorWithRed:255/255.0 green:210/255.0 blue:0/255.0 alpha:1]
...
@interface MNCalendar (DataSourceAndDelegate)
...
- (MNCalendarCellShape)preferedCellShapeForDate:(NSDate *)date;
...
@interface MNCalendar ()<UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
{
    MNCalendarAppearance *_appearance;
    NSMutableArray *_selectedDates;
    NSDate *_minimumDate;
    NSDate *_maximumDate;
    CGFloat  kFSCalendarDefaultHeaderHeight;
    CGFloat  kFSCalendarDefaultStickyHeaderHeight;
    CGFloat  kFSCalendarMinimumRowHeight;
    CGFloat  kFSCalendarDefaultWeekHeight;
}
...
@property (weak  , nonatomic) MNCalendarHeader           *header;
@property (weak  , nonatomic) MNCalendarHeaderTouchDeliver *deliver;
...
//@property (weak  , nonatomic) UIView                     *selectedDayView;
//@property (weak  , nonatomic) UIView                     *eventLineView;
//@property (weak  , nonatomic) UIView                     *recordLineView;
@property (weak  , nonatomic) UIButton                     *backTodayButton;
@property (weak  , nonatomic) UIButton                     *cancelButton;


@property (readonly, nonatomic) id<MNCalendarDelegateAppearance> delegateAppearance;
...
- (void)initialize
{
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        kFSCalendarDefaultHeaderHeight = 60;
    } else {
        kFSCalendarDefaultHeaderHeight = 40;
    }
    kFSCalendarDefaultStickyHeaderHeight = 75;
    kFSCalendarMinimumRowHeight = 40;
    kFSCalendarDefaultWeekHeight = 25;
    
    _appearance = [[MNCalendarAppearance alloc] init];
    ...
    _scrollDirection = MNCalendarScrollDirectionHorizontal;
    _firstWeekday = [_calendar firstWeekday];
    _scope = MNCalendarScopeMonth;
    ...
    [collectionView registerClass:[MNCalendarCell class] forCellWithReuseIdentifier:@"cell"];
    ...
    //    // init controlView
    //    UIView *view = [[UIView alloc] initWithFrame:CGRectZero];
    //    view.backgroundColor = kBlueBg;
    //    [self addSubview:view];
    //    self.controlView = view;
    //
    //    UIView *todayView = [[UIView alloc] initWithFrame:CGRectMake(10, 10, 2*CIRCLERADIUS, 2*CIRCLERADIUS)];
    //    todayView.backgroundColor = kPink;
    //    todayView.layer.cornerRadius = CIRCLERADIUS;
    //    todayView.layer.masksToBounds = YES;
    //    [self.controlView addSubview:todayView];
    //    self.todayView = todayView;
    //
    //    UIView *selectDayView = [[UIView alloc] initWithFrame:CGRectMake(80, 10, 2*CIRCLERADIUS, 2*CIRCLERADIUS)];
    //    selectDayView.backgroundColor = kBlue;
    //    selectDayView.layer.cornerRadius = CIRCLERADIUS;
    //    selectDayView.layer.masksToBounds = YES;
    //    [self.controlView addSubview:selectDayView];
    //    self.selectedDayView = selectDayView;
    //
    //    UIView *recordLineView = [[UIView alloc] initWithFrame:CGRectZero];
    //    recordLineView.backgroundColor = kBlue;
    //    [self.controlView addSubview:recordLineView];
    //    self.recordLineView = recordLineView;
    //
    //    UIView *eventLineView = [[UIView alloc] initWithFrame:CGRectZero];
    //    eventLineView.backgroundColor = kPink;
    //    [self.controlView addSubview:eventLineView];
    //    self.eventLineView = eventLineView;
    ...
    todayBtn.frame = CGRectZero;
    [todayBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [todayBtn setTitle:NSLocalizedString(@"mcs_today", nil) forState:UIControlStateNormal];
    todayBtn.backgroundColor = styleColor;
//    todayBtn.layer.cornerRadius = 5.0;
//    todayBtn.layer.borderWidth = 1.0;
    todayBtn.layer.borderColor = [UIColor whiteColor].CGColor;
    todayBtn.titleLabel.font = [UIFont systemFontOfSize:(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) ? 20 : 16];
    [self addSubview:todayBtn];
    self.backTodayButton = todayBtn;
    [self.backTodayButton addTarget:self action:@selector(backTodayOperation) forControlEvents:UIControlEventTouchUpInside];
    
    UIButton *certainBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    certainBtn.frame = CGRectZero;
    [certainBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [certainBtn setTitle:NSLocalizedString(@"mcs_cancel", nil) forState:UIControlStateNormal];
    certainBtn.backgroundColor = styleColor;
//    certainBtn.layer.cornerRadius = 5.0;
//    certainBtn.layer.borderWidth = 1.0;
    certainBtn.layer.borderColor = [UIColor whiteColor].CGColor;
    certainBtn.titleLabel.font = [UIFont systemFontOfSize:(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) ? 20 : 16];
    [self addSubview:certainBtn];
    self.cancelButton = certainBtn;
    [self.cancelButton addTarget:self action:@selector(certainOperation) forControlEvents:UIControlEventTouchUpInside];
...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        //        _controlView.frame = CGRectMake(0, self.fs_height - 48, self.fs_width, 48);
        //        _todayView.frame = CGRectMake(10, self.controlView.frame.size.height/2 - CIRCLERADIUS, 2*CIRCLERADIUS, 2*CIRCLERADIUS);
        //        _selectedDayView.frame = CGRectMake(80, self.controlView.frame.size.height/2 - CIRCLERADIUS, 2*CIRCLERADIUS, 2*CIRCLERADIUS);
        //        _recordLineView.frame = CGRectMake(150, self.controlView.frame.size.height/2, LINELENGTH, 1);
        //        _eventLineView.frame = CGRectMake(220, self.controlView.frame.size.height/2, LINELENGTH, 1);
        _backTodayButton.frame = CGRectMake(10, _header.frame.size.height/2 - kFSCalendarDefaultHeaderHeight/2 , kFSCalendarDefaultHeaderHeight*1.5, kFSCalendarDefaultHeaderHeight);
        _cancelButton.frame = CGRectMake(_header.frame.size.width - kFSCalendarDefaultHeaderHeight*1.5 - 10, _header.frame.size.height/2 - kFSCalendarDefaultHeaderHeight/2, kFSCalendarDefaultHeaderHeight*1.5, kFSCalendarDefaultHeaderHeight);
    }
...
{
    MNCalendarCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
    cell.calendar = self;
...
//    NSLog(@"%ld",[cell.date fs_day]);
    [self invalidateAppearanceForCell:cell];
    //flag date has record
    NSDate *cellDate = [self extractDate:cell.date];
    [cell.flagView setHidden:YES];
    if ([self.allDatesArray lastObject]) {
        for (calendar_info_obj *calendar_info in self.allDatesArray)
        {
            if ([calendar_info.date timeIntervalSince1970] == ([cellDate timeIntervalSince1970] + 24*60*60))
            {
                [cell.flagView setHidden:NO];
                if (calendar_info.flag) {
                    cell.flagView.backgroundColor = kYello;
                }
            }
        }
    }
...
{
    MNCalendarCell *cell = (MNCalendarCell *)[collectionView cellForItemAtIndexPath:indexPath];
    ...
    if (!self.floatingMode) {
        [collectionView.visibleCells enumerateObjectsUsingBlock:^(MNCalendarCell *cell, NSUInteger idx, BOOL *stop) {
            if (cell.dateIsPlaceholder) {
                cell.dateIsSelected = [self.selectedDates containsObject:cell.date];
                cell.dateIsToday = [cell.date fs_isEqualToDateForDay:_today];
                [cell setNeedsLayout];
...
- (void)setAppearance:(MNCalendarAppearance *)appearance
{
...
- (MNCalendarAppearance *)appearance
{
    return _appearance;
}

- (void)setScrollDirection:(MNCalendarScrollDirection)scrollDirection
{
...
- (void)setScope:(MNCalendarScope)scope
{
...
- (void)setDataSource:(id<MNCalendarDataSource>)dataSource
{
...
- (id<MNCalendarDelegateAppearance>)delegateAppearance
{
    if (_delegate && [_delegate conformsToProtocol:@protocol(MNCalendarDelegateAppearance)]) {
        return (id<MNCalendarDelegateAppearance>)_delegate;
    }
    return nil;
}
...
[self didChangeValueForKey:@"scope"]; \
        ...
        MNCalendarScope prevScope = _scope;
        ...
        BOOL weekToMonth = prevScope == MNCalendarScopeWeek && scope == MNCalendarScopeMonth;
        NSInteger rowNumber = -1;
        
        switch (prevScope) {
            case MNCalendarScopeMonth: {
                MNCalendarCell *cell = (MNCalendarCell *)[_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (void)deselectDate:(NSDate *)date
{
    ...
    if ([_collectionView.indexPathsForSelectedItems containsObject:indexPath]) {
        ...
        MNCalendarCell *cell = (MNCalendarCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        cell.dateIsSelected = NO;
        [cell setNeedsLayout];
        
        [_collectionView.visibleCells enumerateObjectsUsingBlock:^(MNCalendarCell *cell, NSUInteger idx, BOOL *stop) {
...
//        [NSException raise:@"selectedDate out of range" format:@""];
    ...
    if (forPlaceholder) {
        ...
        // Cross month selected date, need to trigger all kinds of events
        if (self.allowsMultipleSelection && [self isDateSelected:targetDate]) {
            // In propagation mode, click on the date of the month has been selected
        ...
        // When you select a date manually, you need to select the date you have chosen, but not the event
        if (self.selectedDate && !self.allowsMultipleSelection) {
            NSIndexPath *currentIndexPath = [self indexPathForDate:self.selectedDate];
            MNCalendarCell *cell = (MNCalendarCell *)[_collectionView cellForItemAtIndexPath:currentIndexPath];
            cell.dateIsSelected = NO;
            [cell setNeedsLayout];
            _daysContainer.clipsToBounds = NO;
        ...
        MNCalendarCell *cell = (MNCalendarCell *)[_collectionView cellForItemAtIndexPath:targetIndexPath];
        _daysContainer.clipsToBounds = NO;
        [cell performSelecting];
...
- (void)invalidateLayout
{
    if (!self.floatingMode) {
        ...
        if (!_header) {
            ...
            MNCalendarHeader *header = [[MNCalendarHeader alloc] initWithFrame:CGRectZero];
        ...
        if (_scrollEnabled) {
            if (!_deliver) {
                MNCalendarHeaderTouchDeliver *deliver = [[MNCalendarHeaderTouchDeliver alloc] initWithFrame:CGRectZero];
        ...
        if (!_weekdays.count) {
            ...
            NSArray *weekSymbols = date.shortStandaloneWeekdaySymbols;
...
//    }];
    ...
    NSDateFormatter *date = [[NSDateFormatter alloc] init];
    [date setCalendar:calendar];
    NSArray *weekdayVeryShort  = date.veryShortStandaloneWeekdaySymbols;
    NSArray *weekdayShort = date.shortStandaloneWeekdaySymbols;
    NSArray *weekdaySymbols = _appearance.useVeryShortWeekdaySymbols ? weekdayVeryShort : weekdayShort;
    [_weekdays enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger index, BOOL *stop) {
        label.text = weekdaySymbols[index];
    }];
}
...
- (MNCalendarCellShape)preferedCellShapeForDate:(NSDate *)date
{
    if (self.delegateAppearance && [self.delegateAppearance respondsToSelector:@selector(calendar:appearance:cellShapeForDate:)]) {
        MNCalendarCellShape cellShape = [self.delegateAppearance calendar:self appearance:self.appearance cellShapeForDate:date];
        return cellShape;
    }
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    else if (self.delegateAppearance && [self.delegateAppearance respondsToSelector:@selector(calendar:appearance:cellStyleForDate:)]) {
        MNCalendarCellShape cellShape = (MNCalendarCellShape)[self.delegateAppearance calendar:self appearance:self.appearance cellStyleForDate:date];
        return cellShape;
    }
#pragma GCC diagnostic pop
    
    return MNCalendarCellShapeCircle;
}
...
@implementation MNCalendar (Deprecated)
...
- (void)setFlow:(MNCalendarFlow)flow
{
    self.scrollDirection = (MNCalendarScrollDirection)flow;
}

- (MNCalendarFlow)flow
{
    return (MNCalendarFlow)self.scrollDirection;
}
...
@implementation calendar_info_obj
