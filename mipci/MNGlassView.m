...
#import "MNDisplayBridge.h"

@interface MNGlassView () <LFDisplayBridgeTriggering>
...
- (void) dealloc {
    ...
    [[MNDisplayBridge sharedInstance] removeSubscribedViewsObject:self];
}
...
- (void) subscribeViewWithBounds:(CGRect)bounds {
    if (!CGRectIsEmpty(bounds) && self.superview) {
        [[MNDisplayBridge sharedInstance] addSubscribedViewsObject:self];
    } else {
        [[MNDisplayBridge sharedInstance] removeSubscribedViewsObject:self];
    }
}
