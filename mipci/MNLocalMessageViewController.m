...
#import "MNLocalMessageViewController.h"
#import "mipc_agent.h"
...
@interface MNLocalMessageViewController ()
...
@implementation MNLocalMessageViewController
...
//    [self initUI];
    ...
    _originLocalMessages = [self loadLocalVideoInfoByID:_deviceID];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:NSStringFromClass([MNRecordViewController class])])
    {
        ...
        recordViewController.boxID = _deviceID;
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self performSegueWithIdentifier:NSStringFromClass([MNRecordViewController class]) sender:videoInfo];
}
...
- (NSString *)checkFilePath:(NSString *)url
{
    ...
    fileString = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:[NSString stringWithFormat:@"video-mp4/%@/%@",_deviceID,fileString]];
    
    //Change String
    NSString *tmpString = [fileString stringByReplacingOccurrencesOfString:@" " withString:@"_"];
    fileURL = tmpString;
